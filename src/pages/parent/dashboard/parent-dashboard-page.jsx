...
import { useParentDashboard } from "@/modules/parent";
import { LoadingFallback, RouteState } from "@/shared/ui";

const metricIcons = { children: UsersRound, requests: Hourglass, lessons: CheckCircle2, minutes: Timer };
...
export function ParentDashboardPage() {
  ...
  return (
    <div className="portal-page">
      <section className="portal-welcome parent-welcome"><div><span className="portal-eyebrow">OTA-ONA KABINETI</span><h1>Xayrli tong, {user?.name?.split(" ")[0]}! 👋</h1><p>Farzandingizning ta’lim jarayonini kuzatib boring.</p></div><div className="welcome-orbit"><UsersRound size={29} /></div></section>
      ...
      <section className="portal-card parent-recent-card"><div className="portal-section-head"><div><span>SO‘NGGI FAOLLIK</span><h2>Davomat</h2></div><Link to={ROUTES.parent.attendance}>Barcha davomat <ArrowRight size={15} /></Link></div><div className="parent-activity-list">{attendance.data.slice(0, 5).map((item) => <article key={item.id}><span><CalendarCheck2 size={18} /></span><div><strong>{item.lesson}</strong><small>{item.child} · {item.entered}</small></div><time><Clock3 size={14} /> {item.duration}</time></article>)}</div></section>
    </div>
  );
}
