...
const metricIcons = { courses: BookOpen, lessons: CalendarClock, assignments: ListChecks, progress: TrendingUp };

export function StudentDashboardPage() {
  const { user } = useAuth();
  const dashboard = useStudentDashboard();
  if (dashboard.isLoading) return <LoadingFallback label="O‘quvchi paneli yuklanmoqda" />;
  if (dashboard.isError) return <RouteState title="Ma’lumotlarni yuklab bo‘lmadi" actionLabel="Qayta urinish" onAction={dashboard.refetch} />;
