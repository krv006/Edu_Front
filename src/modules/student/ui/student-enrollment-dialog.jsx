...
import { useRequestDirect, useTeachersForDirect } from "@/modules/conversation";
...
export function StudentEnrollmentDialog({ open, onOpenChange }) {
  ...
  async function requestTeacher(teacher) { if (teacher.roomId && teacher.directStatus === "active") { onOpenChange(false); navigate(`/student/chats/${teacher.roomId}`); return; } const room = await direct.mutateAsync(teacher.id); onOpenChange(false); if (room.directStatus === "active") navigate(`/student/chats/${room.id}`); }
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>{open ? <DialogContent title="Yangi muloqot" description="O‘qituvchiga so‘rov yuboring yoki ochiq kursga qo‘shiling."><motion.div className="student-enrollment-dialog" initial={{ opacity: 0, y: 7 }} animate={{ opacity: 1, y: 0 }}>
    ...
    <span className="dialog-section-label">O‘QITUVCHIGA YOZISH</span><div className="student-enrollment-list">{(teachers.data ?? []).map((teacher) => <article key={teacher.id}><Avatar name={teacher.name} tone="violet" size="md" /><div><strong>{teacher.name}</strong><small>@{teacher.username} · {teacher.directStatus || "so‘rov yuborilmagan"}</small></div><Button size="sm" disabled={teacher.directStatus === "blocked"} loading={direct.isPending} onClick={() => requestTeacher(teacher)}><MessageCircle size={16} /> {teacher.directStatus === "active" ? "Ochish" : "So‘rov"}</Button></article>)}</div>
