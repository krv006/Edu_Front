...
export function AttentionCheckDialog({ lessonId, check }: AttentionCheckDialogProps) {
  ...
  return <Dialog open={Boolean(check)} onOpenChange={() => undefined}>{check ? <DialogContent title={t("attentionCheck.title")} description={t("attentionCheck.description")} closeable={false}><motion.div className="attention-check" initial={{ scale: .94, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}><ShieldCheck size={36} /><strong>{seconds}</strong><p>{t("attentionCheck.secondsLeft")}</p><Button disabled={seconds <= 0} loading={answer.isPending} onClick={() => answer.mutate(check.id)}>{t("attentionCheck.respond")}</Button></motion.div></DialogContent> : null}</Dialog>;
}
